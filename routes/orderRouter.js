...
orderRouter.get('/edit/:id', Verify, VerifyRole(), async (req, res) => {
  ...
  try {
    ...
    // Validate and filter starting order to only include valid entries
...
orderRouter.get('/createOrder/:id', Verify, VerifyRole(), async (req, res) => {
    ...
    try {
        ...
        if (!timetablePart.conflictsChecked) {
            // Check and generate conflicting orders
            ...
            // Generate complete starting order for all remaining entries
